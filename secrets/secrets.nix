let
  # user@system matrix
  systems = {
    alum = {
      tcarrio = "ecdsa-sha2-nistp256 AAAAE2VjZHNhLXNoYTItbmlzdHAyNTYAAAAIbmlzdHAyNTYAAABBBAl788s2f8cUb8v3pyqX1sv4jRpvBuWeC0pfp+p0Gq1eLKBv9GxENTl146PoqAMkXQB2CJE8KX7vbjZrrirJBBc= GitHub@secretive.alum.local";
    };
    glass = {
      host = "ssh-rsa  root@glass";
      tcarrio = "ssh-rsa  tom@carrio.dev";
    };
    kuroi = {
      host = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIAT4Z64F9AhDZ60xzv9TOAf/+PJQKVQ+hf8ar3nEzpmm root@kuroinixu";
    };
  };
in
{
  "users/tcarrio/ssh.age".publicKeys = with systems.glass; [ tcarrio host ];
  "services/tailscale/token.age".publicKeys = with systems.glass; [ tcarrio host ];
}