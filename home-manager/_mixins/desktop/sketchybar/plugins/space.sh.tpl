...
{{bin.sketchybar}} -m --animate tanh 0 "${args[@]}"

WIN=$({{bin.yabai}} -m query --spaces --space $SID | jq '.windows[0]')
HAS_WINDOWS_OR_IS_SELECTED="true"
if [ "$WIN" = "null" ] && [ "$SELECTED" = "false" ];then
  HAS_WINDOWS_OR_IS_SELECTED="false"
fi
{{bin.sketchybar}} --set $NAME background.drawing=$SELECTED icon.highlight=$HAS_WINDOWS_OR_IS_SELECTED
