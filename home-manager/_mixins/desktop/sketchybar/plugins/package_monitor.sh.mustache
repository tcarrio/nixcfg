...
PREV=$({{bin.sketchybar}} --query packages | jq -r .text.label)
{{bin.sketchybar}} --set $NAME label=$LOADING
...
{{bin.sketchybar}} --set $NAME label="$FINAL updates" label.color=0xffc382db

if [ "$FINAL" != "$PREV" ]; then
  {{bin.sketchybar}} --animate tanh 15 --set $NAME label.y_offset=5 label.y_offset=1
fi
