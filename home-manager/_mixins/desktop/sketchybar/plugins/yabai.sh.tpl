...
update() {
  ...
  WINDOW=$({{bin.yabai}} -m query --windows --window)
  CURRENT=$(echo "$WINDOW" | jq '.["stack-index"]')

  args=()
  if [[ $CURRENT -gt 0 ]]; then
    LAST=$({{bin.yabai}} -m query --windows --window stack.last | jq '.["stack-index"]')
...
mouse_clicked() {
  {{bin.yabai}} -m window --toggle float
  update
}

mouse_entered() {
  {{bin.sketchybar}} --set $NAME background.drawing=on 
}

mouse_exited() {
  {{bin.sketchybar}} --set $NAME background.drawing=off 
}
