...
let
  ...
  generateZoneFile =
    ...
    let
      ...
      sortedRecords = builtins.sort (
        a: b: if a.name == b.name then lib.lessThan a.type b.type else lib.lessThan a.name b.name
      ) records;
      ...
      soaNs =
        if soa != null && soa.ns != null then
          soa.ns
        else if builtins.length nsRecords > 0 then
          (builtins.head nsRecords).value
        else
          "ns1.${normalizedZone}";

      soaEmail =
        if soa != null && soa.email != null then
          soa.email
        else if email != null then
          email
        else
          "hostmaster.${normalizedZone}";

      soaSerial =
        if soa != null && soa.serial != null then
          soa.serial
        else if serial != null then
          serial
        else
          1;

      soaRefresh =
        if soa != null && soa.refresh != null then
          soa.refresh
        else if refresh != null then
          refresh
        else
          "3h";

      soaRetry =
        if soa != null && soa.retry != null then
          soa.retry
        else if retry != null then
          retry
        else
          "1h";

      soaExpire =
        if soa != null && soa.expire != null then
          soa.expire
        else if expire != null then
          expire
        else
          "1w";

      soaNegativeCacheTtl =
        if soa != null && soa.negativeCacheTtl != null then
          soa.negativeCacheTtl
        else if negativeCacheTtl != null then
          negativeCacheTtl
        else
          "1h";
      ...
      formatRecord =
        ...
        let
          ...
          value =
            if record.type == "TXT" && !(lib.hasPrefix "\"" record.value) then
              "\"${record.value}\""
            else
              record.value;
        in
...
in
{
      ...
      attrsOf (
        submodule (
          ...
          }:
          {
            ...
            config.file = lib.mkIf (config.records != null) (
              pkgs.writeText "${name}.zone" (generateZoneFile {
                zone = name;
                inherit (config) records;
                ttl = config.zoneTtl;
                inherit (config) soa;
              })
            );
  ...
  config.warnings = lib.mapAttrsToList (
    zoneName: zoneConfig:
  ...
  config.system.checks = lib.mkIf config.services.bind.enable [
    ...
    (pkgs.writeShellScriptBin "bind-zone-validation" ''
      ...
      ${lib.concatMapStringsSep "\n" (
        zoneName:
        let
          zoneConfig = config.services.bind.zones.${zoneName};
        in
        if zoneConfig.records != null then
          let
            zoneFile = pkgs.writeText "${zoneName}-validation.zone" (generateZoneFile {
              zone = zoneName;
              inherit (zoneConfig) records;
